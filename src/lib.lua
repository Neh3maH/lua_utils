...
module.searchers = package.searchers or package.loaders
...
function module.Module:ld(pwd)
	...
	if '.' == pwd then
		module.searchers[#module.searchers + 1] = module.mk_searcher(self.name, pwd)
	end
	pwd = pwd .. '/libs/'
	for _, dep in pairs(self.deps) do
		local path = pwd .. dep
		module.searchers[#module.searchers + 1] = module.mk_searcher(dep, path)
